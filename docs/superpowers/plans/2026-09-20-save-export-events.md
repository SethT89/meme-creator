...
**Rollout order matters:** the migration is backward compatible (the deployed app inserts click events without `kind`, which defaults to `'pick'`), so it goes to production **first, with the owner's explicit OK**; the code is pushed after.
...
- Modify: `src/lib/queries/templates.ts` — shared event insert, `kind` on click logging, new `useLogExport`.
...
begin
  ...
  insert into template_usage_events (template_id) values (tid);            -- kind defaults to 'pick'
  ...
  insert into template_usage_events (template_id, kind) values (tid, 'export');
  select use_count_total, save_count, export_count into a_total, a_save, a_export from templates where id = tid;
  ...
  b_total := a_total; b_save := a_save; b_export := a_export;
  select count(*) into save_events_before from template_usage_events where kind = 'save' and template_id = tid;
  insert into creations (source_type, template_id, name) values ('template', tid, 'verify-template') returning id into cid;
  select use_count_total, save_count, export_count into a_total, a_save, a_export from templates where id = tid;
  select count(*) into save_events_after from template_usage_events where kind = 'save' and template_id = tid;
  ...
  select count(*) into null_saves_before from template_usage_events where kind = 'save' and template_id is null;
  ...
  select count(*) into null_saves_after from template_usage_events where kind = 'save' and template_id is null;
  ...
  insert into template_usage_events (template_id, kind) values (null, 'export');
  ...
  select count(*) into expected_7d from template_usage_events
    where template_id = tid and kind = 'pick' and created_at > now() - interval '7 days';
  ...
  -- 8. an unknown kind is rejected
  begin
    insert into template_usage_events (template_id, kind) values (tid, 'bogus');
    raise exception 'FAIL bogus kind was accepted';
...
```sql
select
  (select count(*) from information_schema.columns where table_name = 'template_usage_events' and column_name = 'kind') as kind_col_exists,
...
- [ ] **Step 1: `template_usage_events`** — in `Row` add `kind: string` (alphabetical: after `id`) and change `template_id: string` to `template_id: string | null`. In `Insert` add `kind?: string` and change `template_id: string` to `template_id?: string | null`. In `Update` add `kind?: string` and change `template_id?: string` to `template_id?: string | null`.
...
git commit -m "chore: add usage event kind, nullable template_id, and save/export counters to database types
...
Expected: FAIL — `useLogExport` is not exported, and the pick insert has no `kind`.
...
git commit -m "feat: log usage events by kind and add an export logger
...
- [ ] **Step 1: Ask the owner for an explicit OK.** State plainly: adds a `kind` column and a check constraint, makes `template_id` nullable on the events table, adds two columns to `templates`, replaces two functions, and adds a trigger on `creations`. Backward compatible with the deployed app.
...
```sql
select kind, template_id, user_id, created_at from template_usage_events where kind = 'export' order by created_at desc limit 1;
